...
for i = 1:nfeat
    ...
    gui.features.feat(i).condStat.Position  = [0.9 0.1075+0.85*(i-1)/nfeat+0.75/nfeat 0.05 0.04];
    gui.features.feat(i).thresholdU.Position = [0.02 0.22+0.85*(i-1)/nfeat+.1/nfeat 0.03 0.5/nfeat];
    ...
    thisfeature =gui.data.tracking.features{gui.features.feat(i).featNum};
    MIN = mymin(thisfeature);
    MAX = mymax(thisfeature);
    lim = [MIN MAX];
    if (lim(2)-lim(1))==0
    ...
    set(gui.features.feat(i).axes,'ylim',lim);%figure();plot(gui.data.tracking.features{gui.features.feat(i).ch})
    set(gui.features.feat(i).zeroLine,'ydata',lim);
    uistack(gui.features.feat(i).zeroLine,'top');%KM
...
% if in threshold edit mode
if(strcmpi(gui.annot.activeCh,'thresholded_features'))
    
    mask = getThresholdedFeatureMask(gui);

    % KM should i add the conditional statement code here?
%      [mask,gui] =myAddCondStatementForFeature(gui,thisFeat)
